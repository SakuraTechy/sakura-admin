...
/** UI 自动化步骤执行详情 API。 */
@Tag(name = "自动化管理-UI 自动化步骤执行查询 API")
@RestController
@RequiredArgsConstructor
@RequestMapping("/automation/execution-steps")
public class AutomationUiExecutionStepQueryController {

    private final AutomationUiExecutionQueryService executionQueryService;

    @Operation(summary = "查询步骤执行诊断详情")
    @SaCheckPermission("automation:automationUiScene:get")
    @GetMapping("/{stepExecutionDbId}")
    public AutomationUiExecutionStepDetailResp detail(@PathVariable Long stepExecutionDbId) {
        return executionQueryService.stepDetail(stepExecutionDbId);
    }
}
