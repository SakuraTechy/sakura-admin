...
public class AutomationNodeConfigServiceImpl extends BaseServiceImpl<AutomationNodeConfigMapper, AutomationNodeConfigDO, AutomationNodeConfigResp, AutomationNodeConfigDetailResp, AutomationNodeConfigQuery, AutomationNodeConfigReq> implements AutomationNodeConfigService {
    ...
    public List<AutomationNodeConfigDetailResp> selectByIds(List<Long> ids) {
        List<AutomationNodeConfigDetailResp> list = BeanUtil.copyToList(baseMapper
            .selectByIds(ids), AutomationNodeConfigDetailResp.class);
        list.forEach(item -> {
    ...
    public boolean syncAllNode(Long jenkinsId) {
        ...
        try {
            ...
            List<AutomationNodeConfigDO> automationNodeConfigDOList = baseMapper.lambdaQuery()
                ...
                .eq(AutomationNodeConfigDO::getDelFlag, 3)
                .list();
            List<AutomationEnvironmentConfigDO> automationEnvironmentConfigDOList = automationEnvironmentConfigMapper
                .lambdaQuery()
                .eq(AutomationEnvironmentConfigDO::getDelFlag, 3)
                .list();
            if (StringUtils.isNotEmpty(automationNodeConfigDOList) && jsonNode.get("computer")
                .size() == automationNodeConfigDOList.size()) {
                        ...
                        automationNodeConfigDOList.forEach((automationNodeConfigDO) -> {
                            ...
                            if (automationNodeConfigDO.getName().equals(displayName[0])) {
                                ...
                                if (JsonUtils.isJson(descriptionStr)) {
                                    description = JSON
                                        .parseObject(descriptionStr, AutomationNodeConfigDO.Description.class);
                                } else {
                                ...
                                automationNodeConfigDO.setOfflineStatus(offlineStatus
                                    ? StatusTypeEnum.OFFLINE
                                    : StatusTypeEnum.ONLINE);
                                ...
                                automationNodeConfigDO.setIdleStatus(offlineStatus
                                    ? StatusTypeEnum.OFFLINE
                                    : idleStatus ? StatusTypeEnum.IDLE : StatusTypeEnum.IN_USE);
                                if (!idleStatus) {
                                    ...
                                    for (JsonNode executors : computer.get("executors")) {
                                        ...
                                        if (!currentExecutableJn.isEmpty()) {
                                            ...
                                            if (StringUtils.isNotEmpty(url)) {
                                                ...
                                                currentExecutable.setUser(JenkinsService
                                                    .getJenkinsJobParameters(url, jenkinsUserName, jenkinsPassWord));
                                ...
                                AutomationNodeConfigDO.Config config = new AutomationNodeConfigDO.Config();
                                Response response = JenkinsService
                                    .getJenkinsNodeDetails(jenkinsUrl, jenkinsUserName, jenkinsPassWord, displayName[0]);
                                JsonNode rootNode = JenkinsService.parseXmlToJson(response.xmlPath());
                                if (rootNode != null) {
                                    JsonNode credentialsId = rootNode.get("slave").get("launcher").get("credentialsId");
                                    if (credentialsId != null && !credentialsId.isMissingNode()) {
                                        description.setSystemType("Linux");
                                        description
                                            .setCredentialsId(jenkinsUrl + "/credentials/store/system/domain/_/credential/" + credentialsId
                                                .asText());
                                        automationNodeConfigDO.setType("Linux");
                                    } else {
                                        description.setSystemType("Windows");
                                    ...
                                    configList.add(config);
                                    JsonNode toolLocationNode = rootNode.get("slave")
                                        .get("nodeProperties")
                                        .get("hudson.tools.ToolLocationNodeProperty")
                                        .get("locations")
                                        .get("hudson.tools.ToolLocationNodeProperty_-ToolLocation");
                                    for (JsonNode tools : toolLocationNode) {
                            ...
                            for (AutomationEnvironmentConfigDO automationEnvironmentConfigDO : automationEnvironmentConfigDOList) {
                                List<AutomationNodeConfigDO> nodeConfigList = automationEnvironmentConfigDO
                                    .getNodeConfig();
                ...
                for (JsonNode computer : jsonNode.get("computer")) {
                    ...
                    try {
                        ...
                        AutomationNodeConfigDO.Config config = new AutomationNodeConfigDO.Config();
                        Response response = JenkinsService
                            .getJenkinsNodeDetails(jenkinsUrl, jenkinsUserName, jenkinsPassWord, displayName);
                        JsonNode rootNode = JenkinsService.parseXmlToJson(response.xmlPath());
                        if (rootNode != null) {
                            JsonNode credentialsId = rootNode.get("slave").get("launcher").get("credentialsId");
                            if (credentialsId != null && !credentialsId.isMissingNode()) {
    ...
    public boolean syncNode(List<Long> ids) {
        try {
            List<AutomationNodeConfigDO> list = BeanUtil.copyToList(baseMapper
                .selectByIds(ids), AutomationNodeConfigDO.class);
            for (AutomationNodeConfigDO item : list) {
                AutomationJenkinsConfigDetailResp jenkinsConfigDetailResp = automationJenkinsConfigService.get(item
                    .getJenkinsId());
                String jenkinsUrl = jenkinsConfigDetailResp.getUrl();
                String jenkinsUserName = jenkinsConfigDetailResp.getUserName();
                String jenkinsPassWord = jenkinsConfigDetailResp.getPassWord();

                JsonNode computer = JenkinsService.getJenkinsNode(jenkinsUrl, jenkinsUserName, jenkinsPassWord, item
                    .getName());
                final String[] displayName = {computer.path("displayName").asText()};
                if (item.getName().equals(displayName[0])) {
                    try {
                        ...
                        if (item.getName().equals(displayName[0])) {
                            ...
                            boolean idleStatus = computer.get("idle").asBoolean();
                            idle.setStatus(offlineStatus ? 6 : idleStatus ? 7 : 8);
                            item.setIdleStatus(offlineStatus
                                ? StatusTypeEnum.OFFLINE
                                : idleStatus ? StatusTypeEnum.IDLE : StatusTypeEnum.IN_USE);
                            if (!idleStatus) {
                                List<AutomationNodeConfigDO.Active.Idle.CurrentExecutable> currentExecutableList = new ArrayList<>();
    ...
    public boolean addNode(AutomationNodeConfigDO automationNodeConfigDO) {
        try {
            ...
            if (JenkinsService
                .addJenkinsNode(jenkinsUrl, jenkinsUserName, jenkinsPassWord, nodeName, automationNodeConfigDO
                    .getType(), automationNodeConfigDO.getJson())) {
                JsonNode computer = JenkinsService
                    .getJenkinsNode(jenkinsUrl, jenkinsUserName, jenkinsPassWord, nodeName);
                ...
                AutomationNodeConfigDO.Config config = new AutomationNodeConfigDO.Config();
                Response response = JenkinsService
                    .getJenkinsNodeDetails(jenkinsUrl, jenkinsUserName, jenkinsPassWord, nodeName);
                JsonNode rootNode = JenkinsService.parseXmlToJson(response.xmlPath());
                if (rootNode != null) {
                    try {
                        JsonNode credentialsId = rootNode.get("slave").get("launcher").get("credentialsId");
