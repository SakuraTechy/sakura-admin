...
public class AutomationStorageCleanupJob {
    ...
    private int delete(String table, String timeColumn, int retentionDays, boolean terminalOnly) {
        ...
        String terminalClause = terminalOnly
            ? " AND LOWER(status) IN ('completed','passed','failed','cancelled','canceled','interrupted','blocked','skipped')"
            : "";
    ...
    private ExecutionCleanupResult deleteExpiredExecutionDetails() {
        ...
        List<Long> ids = jdbcTemplate
            .query("SELECT e.id FROM automation_ui_execution e WHERE e.retention_hold = 0 AND LOWER(e.status) IN ('completed','passed','failed','cancelled','canceled','interrupted','blocked','skipped')" + " AND e.metric_time < DATE_SUB(CURRENT_TIMESTAMP, INTERVAL " + days + " DAY)" + " ORDER BY e.id LIMIT " + BATCH_SIZE, (rs,
                                                                                                                                                                                                                                                                                                                                    rowNum) -> rs
                                                                                                                                                                                                                                                                                                                                        .getLong(1));
        if (ids.isEmpty()) {
            return new ExecutionCleanupResult(0, 0, 0);
