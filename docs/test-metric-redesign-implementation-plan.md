...
8. 底部展示精确、推断、缺失维度质量。
9. 页面在主布局的固定高度内容区内独立纵向滚动，禁止横向溢出；桌面筛选栏按可用宽度换行，移动端改为单列。
10. 当前范围无执行数据时保留 KPI 和结果汇总，仅展示一个紧凑空状态，不渲染零数据趋势、分布、失败排行和维度质量；有数据时完整展示全部区域。
...
- 浏览器分别以空数据和有数据模拟接口实测 1674×870、1440×900、390×844；三个视口均无页面内容重叠或横向溢出，并可滚动到最后的维度质量区域。
- 空数据状态不创建 ECharts 画布，通过率和平均耗时无样本时显示 `--`；有数据状态的 3 个 ECharts 画布在桌面和移动视口均有非空像素。
